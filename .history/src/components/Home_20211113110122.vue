...
.container.mx-auto.p-4
	.w-full.card.-mx-2.overflow-hidden
		.card-body
			.card-title
...
async function generate() {
	...
	switch (options.position) {
		case 'lowerRight':
			opsiposisi = watermark.text.lowerRight
			break;
		case 'lowerLeft':
			opsiposisi = watermark.text.lowerLeft
			break;
		case 'upperRight':
			opsiposisi = watermark.text.upperRight
			y = 48
			break;
		case 'upperLeft':
			opsiposisi = watermark.text.upperLeft
			y = 48
			break;
		case 'center':
			opsiposisi = watermark.text.center
			break;
		default:
			opsiposisi = watermark.text.lowerRight
			break;
	...
	preview.value = await Promise.all(
		// files.value.map(async (file) => await watermark([file]).dataUrl(opsiposisi(options.text, options.textSize+'em '+(options.font || 'Calibri'), (options.colour.hex || options.colour || '#000'), 0,5, y)).then((url) => url))
		files.value.map(async (file) => await watermark([file]).dataUrl(watermark.text.lowerRight(options.text, '48px Calibri', (options.colour.hex || options.colour || '#000'), 0.5)).then((url) => url))
	)
}
</script>