...
.container.mx-auto.p-4
	.flex.flex-wrap.-mx-2.overflow-hidden
		...
		div(class="sm:w-3/3 overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					.card-title
						h1 {{text}}
...
watch(files, async () => {
	...
	preview.value = await Promise.all(
		files.value.map(async (file) => await watermark([file]).dataUrl(opsi(options.text, options.textSize+'em '+options.font, options.colour, 0,5, y)).then((url) => url))
	)
})
