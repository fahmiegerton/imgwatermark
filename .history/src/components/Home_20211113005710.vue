...
watch(files, async () => {
	...
	preview.value = await Promise.all(
		files.value.map(async (file) => watermark(['https://i.picsum.photos/id/430/536/354.jpg?hmac=uxrNCXgJuycp2JMZ9jpZb5ThTsZIplRTSPuc4ybMyws']).image(watermark.image.lowerRight(options.text, 0,5)).then((img) => img))
	)
})
