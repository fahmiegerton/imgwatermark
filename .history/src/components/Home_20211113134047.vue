		...
		div(class="sm:1/2 overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					...
					button.btn.btn-primary(:disabled="preview.length < 1" @click="download") Unduh 🔥
...
function download() {
	// console.log(preview.value[0].split(',')[1])
	const filename = makeid(20)+'-imgwatermark.jpg'

	// var image_data = atob(preview.value[0].split(',')[1]);
	// const image = preview.value[0].split(',')[1]
	// const uri = window.URL.createObjectURL(preview.value[0])
	const uri = preview.value[0]
