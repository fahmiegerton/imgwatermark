...
async function generate() {
	...
	preview.value = await Promise.all(
		...
		files.value.map(async (file) => await watermark([file]).dataUrl(watermark.text[opsiposisi](options.text, '48px Calibri', (options.colour.hex || options.colour || '#000'), 0.5)).then((url) => url))
	)
}
</script>