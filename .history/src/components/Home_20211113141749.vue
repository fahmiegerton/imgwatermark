		...
		div(class="sm:1/2 overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					...
					button.btn.my-2(@click="generate" :disabled="preview.length < 1" :class="preview.length < 1 ? 'btn-secondary' : 'btn-secondary'") Terapkan

					form(@submit.prevent="download" :aria-disabled="preview.length < 1 || !options.filename")
						.form-control
							label.label
								span.label-text Nama Berkas
							input.input.input-bordered(type="text" v-model="options.filename" :disabled="preview.length < 1" required)
						
						.form-control
							label.label
								span.label-text Jenis Berkas
							select.select.select-bordered(v-model="options.filetype" :disabled="preview.length < 1")
								option(disabled="disabled" :selected="!options.filetype") Pilih filetype
								option(v-for="(filetype, index) in filetypes" :key="index" :selected="options.filetype === filetype") {{filetype}}

						button.btn.btn-primary.mt-2.w-full(:disabled="preview.length < 1 || !options.filename" type="submit") Unduh 🔥
...
function download() {
	if (options.filename === '' || options.filename === null) options.filename = makeid(15)
	const filename = options.filename+'-imgwatermark.'+(options.filetype || 'jpg')
	const link = document.createElement('a')
	link.href = preview.value[0]
	...
	link.click()
	link.remove()
	window.URL.revokeObjectURL(preview.value[0])
}
</script>