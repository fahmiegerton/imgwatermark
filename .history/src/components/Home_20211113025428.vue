		...
		div(class="sm:1/2 overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					...
					button.btn.btn-secondary.mt-2(@click="refreshimage") Apply
...
async function generate() {
	...
	console.log(opsiposisi)

	preview.value = await Promise.all(
		files.value.map(async (file) => await watermark([file]).dataUrl(watermark.text[options.position](options.text, options.textSize+'em '+(options.font || 'Calibri'), (options.colour.hex || options.colour || '#000'), 0,5)).then((url) => url))
	)
}
