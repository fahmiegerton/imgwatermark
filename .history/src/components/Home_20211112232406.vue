...
.container.mx-auto.p-4
	.flex.flex-wrap.-mx-2.overflow-hidden
		div(class="sm:w-auto overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					...
					.form-control
						...
						input.input.input-bordered(type="text" v-model="opts.text")
		div(class="sm:w-4/5 overflow-hidden my-2 px-2")
			.card.shadow-2xl.bg-green-400
				.card-body
					.card-title
						h1 {{text}}
					...
					div(v-if="files.length > 0")
						...
						img(:src="preview[0]" alt="gambar"  v-water-mark="opts")
...
const files = ref([])
const opts = ref({
	text: 'Sample', textColor: 'rgba(0, 0, 0, 0.5)', font: '20px Segoe UI', row: 100, col: 170 
})
