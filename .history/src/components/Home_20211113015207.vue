			...
			.card.shadow-2xl.bg-green-400
				.card-body
					...
					.form-control
						...
						vue-tailwind-color-picker(v-model="options.colour" :swatches.sync="swatches")
...
watch(files, async () => {
	...
	preview.value = await Promise.all(
		files.value.map(async (file) => await watermark([file]).dataUrl(opsi(options.text, options.textSize+'em '+options.font, 0,5, y)).then((url) => url))
	)
})
