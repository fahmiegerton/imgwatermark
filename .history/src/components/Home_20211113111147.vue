...
async function generate() {
	...
	preview.value = await Promise.all(
		...
		files.value.map(async (file) => await watermark([file]).dataUrl(watermark.text[opsiposisi](options.text, options.textSize+'em '+(options.font || 'Calibri'), (options.colour.hex || options.colour || '#000'), 0.5)).then((url) => url))
	)
}
</script>